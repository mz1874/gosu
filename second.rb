...
class GameWindow < Gosu::Window
  ...
  def initialize
    ...
    while line_index < @columns.length
      # 获取每一列
      line = @columns[line_index]
      # 列索引
      row_index = 0
      puts "------------------------------------"
      while row_index < line.length
        cell = @columns[line_index][row_index]
        # 上
        if row_index != 0
          cell.north = @columns[line_index][row_index - 1]
        end
        # 下
        if row_index != ((MAP_WIDTH / CELL_DIM) - 1)
          cell.south = @columns[line_index][row_index + 1]
        end

        # 左
        if line_index != 0
          cell.west = @columns[line_index-1][row_index]
        end

        # 右
        if line_index != ((MAP_WIDTH / CELL_DIM) - 1)
          cell.east = @columns[line_index + 1][row_index]
        end

        puts "X #{line_index} \t Y #{row_index} \t North :#{@columns[line_index][row_index].north == nil ? 0 :1}\t South:   #{@columns[line_index][row_index].south == nil ? 0 :1} \t Wast: #{@columns[line_index][row_index].west == nil ? 0 :1} \t East: #{@columns[line_index][row_index].east == nil ? 0 :1}"
        row_index += 1
      end
      line_index += 1
    end
